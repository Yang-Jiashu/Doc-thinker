...
class ProcessorMixin:
    ...
    async def _process_multimodal_content_individual(
        ...
        if all_chunk_results:
            ...
            workspace = getattr(self.lightrag, "workspace", None)
    ...
    async def _process_multimodal_content_batch_type_aware(
        ...
        extraction_mode = (self.config.relation_extraction_mode or "lightrag").lower()
        if extraction_mode not in {"lightrag", "hypergraph"}:
            self.logger.warning(
                "Unknown relation_extraction_mode '%s', falling back to LightRAG.",
                extraction_mode,
            )
            extraction_mode = "lightrag"

        chunk_results: List[Tuple] | None = None
        if extraction_mode == "hypergraph":
            await self._batch_extract_entities_hypergraph_prompt(
                lightrag_chunks, doc_id=doc_id, file_path=file_path
            )
        else:
        ...
        if chunk_results is not None:
            ...
            self.logger.info(
                "HyperGraph prompt-based extraction selected; skipping LightRAG merge pipeline."
            )
    ...
    async def _batch_extract_entities_lightrag_style_type_aware(
        ...
        concurrency = max(1, int(getattr(self.lightrag, "llm_model_max_async", 8)))
            ...
            async with semaphore:
                ...
                for attempt in range(attempts):
                    try:
                        ...
                        # Use a shallow copy of LightRAG config and override only llm_model_func
                        alt_global_cfg = dict(self.lightrag.__dict__)
        ...
        self.logger.info(
            f"Extracted entities from {len(all_results)} results across {len(lightrag_chunks)} multimodal chunks"
        )
        ...
        if hasattr(self, 'add_knowledge_entry'):
            for result in all_results:
                ...
                if chunk_id and chunk_id in lightrag_chunks:
                    chunk = lightrag_chunks[chunk_id]
    ...
    async def _batch_extract_entities_hypergraph_prompt(
        ...
        *,
        ...
        from raganything.hypergraph.operate import extract_entities as hyper_extract_entities

        if self.lightrag is None:
            raise RuntimeError("LightRAG must be initialized before extraction.")

        llm_func = self.llm_model_func or getattr(self.lightrag, "llm_model_func", None)
        if llm_func is None:
            raise RuntimeError(
        ...
        addon_params = dict(getattr(self.lightrag, "addon_params", {}) or {})
        ...
        global_config = {
            ...
            "llm_model_max_token_size": getattr(
                self.lightrag, "llm_model_max_token_size", 4096
            ),
            "tiktoken_model_name": getattr(
                self.lightrag, "tiktoken_model_name", "gpt-4o-mini"
            ),
            "entity_summary_to_max_tokens": getattr(
                self.lightrag, "entity_summary_to_max_tokens", 500
            ),
        ...
        await hyper_extract_entities(
            ...
            knowledge_graph_inst=self.lightrag.chunk_entity_relation_graph,
            entity_vdb=self.lightrag.entities_vdb,
            hyperedge_vdb=self.lightrag.relationships_vdb,
