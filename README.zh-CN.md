...
Expanded-node 的 candidate→active→promoted 生命周期仍是独立的可选记忆能力，不是 ECLRR 隐藏边的审批流程。图谱二中，入库原始关系显示为实线，通过 ECLRR-v4 的 promoted 关系显示为红色虚线。需要制作说明图时，可直接使用[架构图生图 Prompt](docs/ECLRR_V4_ARCHITECTURE_IMAGE_PROMPT.md)。
