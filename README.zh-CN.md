...
- **生长** — 不局限于摄入文档中的显式知识，还能自主推理并延伸出文档未直接表述的隐含关联以及相关知识；
...
- **自进化知识图谱** — 扩展节点以 candidate 状态入图，经用户查询采纳后逐步晋升为正式节点（use_count ≥ 2, score ≥ 1.2），未采纳则衰减；同时后台以滑动窗口扫描实体，自动推断层级、因果、对比等六类潜在边并补全图谱
...
扩展节点以 **candidate** 状态入图，经用户查询采纳后逐步晋升（`candidate → active → promoted`），达到阈值（use_count ≥ 2, score ≥ 1.2）后正式并入图谱，未采纳的节点分数以 0.05 步长衰减。

同时，后台运行**边发现管线**补全图谱结构：

1. **窗口扫描** — 实体按重叠窗口分组（窗口 30，重叠 10）。
2. **LLM 关系推理** — 每个窗口分析 6 类关系：层级、因果、对比、时序、应用、协作。
3. **去重与验证** — 发现的边与已有边对比检查，验证合理性。
4. **可视化区分** — 发现的边以 `is_discovered=1` 持久化，在 KG 可视化中以红色虚线渲染。
