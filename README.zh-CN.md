...
`docthinker.memory_core.AgentMemoryCore` 是当前稳定的 agent 记忆门面。它显式定义了 conversation memory、episodic memory、expanded KG hypotheses、graph promotion 和可选 chat-turn ingestion 的 backend protocols。`MemoryPolicy` 用来控制启用哪些记忆层，以及每层召回多少上下文。回答生成前，`recall()` 会合并：
...
### 7. 📊 记忆与图谱可观测性
Web UI 现在包含 query-time Memory Inspector 和 KG dashboard。它们会展示 recall trace、episodic matches、expanded-node 生命周期、图谱统计、记忆后端状态和证据来源，让用户能看清一个回答为什么生成，以及知识如何随使用被晋升。
