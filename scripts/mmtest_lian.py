...
def build_api_config() -> dict:
    ...
    if not BLTCY_API_KEY or BLTCY_API_KEY.startswith("YOUR_"):
        raise SystemExit(
            "Please set DASHSCOPE_API_KEY or BLTCY_API_KEY (env or .env). See scripts/mmtest_lian.py."
        )
    if not SILICONFLOW_API_KEY or SILICONFLOW_API_KEY.startswith("YOUR_"):
        raise SystemExit(
            "Please set SILICONFLOW_API_KEY (env or .env). See scripts/mmtest_lian.py."
        )
