"""
Shim package to expose the actual LightRAG implementation under lightrag/lightrag.

Upstream installs the project so that `lightrag` is the importable package. When using
the source tree directly (which contains an outer `lightrag` folder that holds the
real package as a subdirectory), we provide this thin wrapper so that
`import lightrag` behaves exactly like the published package.
"""
...
_INNER_PATH = _THIS_DIR / "lightrag"
...
_inner = import_module(".lightrag", __name__)

LightRAG: Any = getattr(_inner, "LightRAG")
